...
/**
 * Class invariant: This code for a binary tree satisfies the binary search tree
 * storage rule. CSSSKL 143
 * 
 * @author (your name)
...
public class CharTree {
	...
	/*
	 * Inner class Node, 2 references(pointers), one data element The only reason
	 * this inner class is static is that it is used in the static methods
	 * insertInSubtree , isInSubtree , and showElementsInSubtree. This class should
	 * have more methods. This is just a sample of possible methods.
	 */
	...
	/**
	 * Returns the root node of a tree that is the tree with root node subTreeRoot,
	 * but with a new node added that contains item.
	 */
	...
	private static boolean isInSubtree(char item, TreeNode subTreeRoot) {
		...
		// else if subTreeRoot.data == item what would you return?
			...
			return true;
		} else if (item < subTreeRoot.data) {// else item < subTreeRoot.data
			return isInSubtree(item, subTreeRoot.leftLink); // recursive call
		} else if (item > subTreeRoot.data) { // else item >= link.data
			return isInSubtree(item, subTreeRoot.rightLink); // recursive call
	...
	public static void main(String[] args) {
		...
		/*
		 * // Remove a node that is not in the tree tree.remove('e');
		 * System.out.println(); tree.showElements(); // Remove a node with one child
		 * tree.remove('a'); System.out.println(); tree.showElements(); // Remove a leaf
		 * node tree.remove('b'); System.out.println(); tree.showElements(); // Remove a
		 * node with two children tree.remove('t'); System.out.println();
		 * tree.showElements(); // Remove root node tree.remove('c');
		 * System.out.println(); tree.showElements();
		 */
	}
	...
	/*
	 * public void remove(char item) { root = removeFromSubtree(item, root); }
	 * 
