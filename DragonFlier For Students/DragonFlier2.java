 ...
 * DragonFlier2
 * ------------
 * DragonFlier (v2) is a video game where you fly a dragon around trying to eat
 * eggs.
...
public class DragonFlier2 {
    // the width and height of the window (the "JFrame") where the video game
    // happens:
    ...
    public DragonFlier2() {
         ...
         * In this kind of program, things don't necessarily happen sequentially,
         * executing line by line.
         * Instead, events occur when they need to, or in response to things you do
         * (like pressing a key on the keyboard),
         * or when a timer runs out. Sometimes it might happen that two events happen
         * relatively simultaneously.
         * For example, a timer goes off that says it's time for an egg to hatch at the
         * same time as you press
         * the space bar to shoot a fireball. So that we don't lose track of any of
         * these events, all events
         * are put into an EventQueue, which then launches them in the order they
         * arrived, as soon as it is able.
         * The invokeLater method essentially puts something into the EventQueue and
         * says "launch this as soon as you are done
         * with any other events you might currently be working on" -- and since we're
         * starting this right at the beginning of
         ...
         * Well, Runnable is an interface that has just one method, the run() method. So
         * here, we're defining and creating this new
         * object of type Runnable, just by giving it the run() method. The Runnable
         * interface is used for creating what are called "threads".
         * The idea behind threads is that they are bits of code that can run on their
         * own, perhaps at the same time as other bits of code.
         * (If your computer has multiple processors, then one thread could be running
         * on one processor, while another thread runs on another.)
         * So these threads (bits of code) are the events that are put into the
         * EventQueue and run in the order they arrive in the queue.
         * In this case, the run method just has stuff setting up the JFrame (window)
         * that the video game will go inside.
         */
        EventQueue.invokeLater(new Runnable() {
            @Override
            public void run() {

                // The UIManager class is used to determine the style of buttons and labels and
                // widgets and whatnot.
                // Here we try to set them so that they'll be consistent with whatever operating
                // system you're using.
                // (E.g., if you're running this on a Mac, the buttons will look like Mac-style
                // buttons,
                // and if you're running it on a Windows box, they'll look like Windows
                // buttons.)
                // However, my program doesn't currently have any buttons or widgets, so this
                // doesn't
                // do much.
                try {
                ...
                // Here we create the main JFrame (or "window") that the game will be contained
                // in,
                // and we set the title and size of the frame, and what should happen if we hit
                // the little 'x'
                ...
                // All the action takes place in the GamePane, and it's the main class where
                // everything happens.
     ...
     * GamePane
     * --------
     * The GamePane class extends the JPanel class, wihch is one way of drawing some
     * stuff inside a JFrame.
     * One thing to know about the JPanel is that it automatically calls the
     * paintComponent method.
     * In our constructor, we also set a timer that will, every 5 miliseconds,
     * change the locations of
     * various things in our video game (like move the dragon up if you've pushed
     * the up arrow key),
     * and then call the repaint() method to clear the screen and call the
     * paintComponent method again.
    ...
    private class GamePane extends JPanel {
         ...
         * --------------------
         * This intializes everything and starts a timer that will move the dragon and
         * background and eggs and fireball every 5 milliseconds
         ...
         * have been defined and their actionPerformed methods are set to do the right
         * thing.
        ...
        public GamePane() {
            ...
            // The initialize method sets everything up: the dragon, the background, the
            // fireball, the eggs.
            // also it does the keyBindings, which binds certain keyboard keys to certain
            // actions
            // (For example, pressing the space bar shoots the fireball, and pressing the up
            // arrow makes you go up)
            initialize();

            // The following is a Swing Timer, which, in this case, creates a new
            // ActionLister object every 5 milliseconds.
            // The ActionListener object will automatically call its actionPerformed method.
            // In this case, the actionPerformed method will:
            // move the dragon, the background, the fireball, and the eggs (moving means
            // changing their x,y coordinates and velocity, etc.)
            // and then check for any collisions (which will update the collided objects
            // accordingly, and change score and health, etc.)
            // Then, after it's updated all the information, it will call the repaint()
            // method (inherited from JPane)
         ...
         * intialize
         * ---------
         * This is used to bring everything in the game to it's original starting
         * position.
         * This could be used in the constructor to set everything up,
         * or it could be used later to reset the game (like if they lose and want to
         * play again)
         * PRE: all required images are in the images folder.
         * (This method creates a dragon as well as the background and fireball and
         * eggs, all of which need images loaded up)
        ...
        private void initialize() {
            ...
            this.dragon = new Dragon(background, FRAME_WIDTH / 2, FRAME_HEIGHT / 2); // dragon's starting location is in
                                                                                     // the center of the frame
            this.fireball = new Fireball(dragon, background, FRAME_WIDTH, FRAME_HEIGHT); // fireball is initially
                                                                                         // invisible
            this.msg = new MessageBox(dragon, background, FRAME_WIDTH, FRAME_HEIGHT,
                    "Use arrow keys to move. Use R key to reset. Use space bar to attack with fire ball. Press I to turn on and off Message Box.");

            loadEggs(); // creates all the eggs and puts them into this.eggList

            // I keep track of how many seconds have elapsed in the game, and I do that by
            // getting an inital startTime.
            this.startTime = System.currentTimeMillis();

            // the keyBindings bind particular keys to Action objects..
            // So for example, if they press the up arrow ("VK_UP"), it will create a new
            // UpAction object,
            // and that object will (because it's a descendent of the Action class), call
            // its
            // ActionPerformed method (because an action -- the key pressing -- just
            // occured).
            // Note that for the arrow keys, there's an action that happens when you press
            // the key,
            ...
            addKeyBinding(KeyStroke.getKeyStroke(KeyEvent.VK_UP, 0, false), "dragon.up.pressed",
                    new UpAction(dragon, true, background));
            addKeyBinding(KeyStroke.getKeyStroke(KeyEvent.VK_UP, 0, true), "dragon.up.released",
                    new UpAction(dragon, false, background));
            addKeyBinding(KeyStroke.getKeyStroke(KeyEvent.VK_DOWN, 0, false), "dragon.down.pressed",
                    new DownAction(dragon, true, background));
            addKeyBinding(KeyStroke.getKeyStroke(KeyEvent.VK_DOWN, 0, true), "dragon.down.released",
                    new DownAction(dragon, false, background));

            addKeyBinding(KeyStroke.getKeyStroke(KeyEvent.VK_LEFT, 0, false), "dragon.left.pressed",
                    new LeftAction(dragon, true, background));
            addKeyBinding(KeyStroke.getKeyStroke(KeyEvent.VK_LEFT, 0, true), "dragon.left.released",
                    new LeftAction(dragon, false, background));
            addKeyBinding(KeyStroke.getKeyStroke(KeyEvent.VK_RIGHT, 0, false), "dragon.right.pressed",
                    new RightAction(dragon, true, background));
            addKeyBinding(KeyStroke.getKeyStroke(KeyEvent.VK_RIGHT, 0, true), "dragon.right.released",
                    new RightAction(dragon, false, background));
            ...
            addKeyBinding(KeyStroke.getKeyStroke(KeyEvent.VK_R, 0, true), "R pressed: restarting game",
                    new R_Action(this));
            addKeyBinding(KeyStroke.getKeyStroke(KeyEvent.VK_I, 0, true), "I pressed: message box",
                    new I_Action(msg));
        }
         ...
         * PRE: none (since you don't call this method directly, you don't need to worry
         * about it)
         * POST: paints a bunch of stuff on the screen:
         * the background, the fireball, the eggs, the dragon, the score, the life left,
         * the seconds elapsed,
        ...
        protected void paintComponent(Graphics g) {
            ...
            // If we thought we might do something to the Graphics object that we wouldn't
            // want to be permenant,
            // We could create a copy of the Graphics object
            // (and maybe cast it to Graphics2d which has some extra methods):
            // Graphics2D g2d = (Graphics2D) g.create(); // later we'll have to dispose of
            // this Graphics object.

            // We pass the Graphics object to the background's draw method, so it can draw
            // itself.

            // To get something (background, dragon, egg, etc.) to be drawn,
            // Each thing has it's own draw method, and we just need to pass the Graphics
            // object to it:
         ...
         * ------------
         * Just what it says!
         * This is meant to be called inside the paintComponent method, since that's
         * where the Graphics object g comes from.
         * PRE: none
         ...
         * (So, for example, if lifeLeft== .25, then the rectangle would be 25% green
         * (on the left side),
         * and 75% red (on the right side).)
         * x & y are where to place the top left corner of the rectangle.
         * This method is meant to be called inside the paintComponent method, since
         * that's where the Graphics object g comes from.
         * PRE: 0 <= lifeLeft <= 1, x & y are in a valid region of the screen (otherwise
         * this might not get seen)
         * POST: displays a rectangle showing how much life you have left next to the
         * word "LIFE:"
         */
         ...
         * Then, any time that keyStroke is detected, it will cause that Action to be
         * created.
         * Learn about keyBindings here:
         * https://docs.oracle.com/javase/tutorial/uiswing/misc/keybinding.html
         * PRE: none of the parameters are null.
         * POST: keyStroke ks will cause Action action to be created (which will call
         * that Action's actionPerformed method).
         */
         ...
         * collisionchecker
         * ----------------
         * This checks for dragon collision with eggs/creatures and fireball collision
         * with eggs:
         * (Note: this method DOES NOT check for collisions with the WALL (i.e,
         * background), since each moving object
         * -- dragon, egg fireball -- deals with possible collisions with the wall in
         * their own move methods.)
         ...
         * If the eggStage is "not ready to eat", then egg is eaten and health goes down
         * by a ten percent.
         * If the eggStage is "hatched", then if it's hungry, it'll reduce dragon's life
         * a bit and then be not hungry for a couple of seconds
         * If the fireball collides with an egg, then (so long as the fireball isn't in
         * smoke form), again we check the stage of the egg:
         * If the eggStage is "hatched", then the creature is shot and the fireball
         * disappears
         * If the egg isn't hatched yet, then you lose a point for destroying an egg
         * that could have been eaten.
         * PRE: loadEggs() has been called
         * POST: eggs, dragon, and fireball have all been updated in light of any
         * possible collisions.
         */
            ...
            for (Egg thisEgg : eggList) {
                ...
                if (eggDragonDistance < eggDragonCollisionDistance) {
                    if (thisEgg.stage == eggStage.READYTOEAT) {
                        ...
                        // just in case the creature is moving slowly so the bounce isn't noticable,
                        // make it a big bounce:
                ...
                if (fireball.hasFired() && fireball.currentFireballImage == fireball.fireballImage) {
                    // first check if the egg is still in egg form (in which case you lose a point
                    // for shooting it):
                    ...
                    // then check if the egg has hatched and is in creature form:
                        ...
                        // this.score++; // killing the creature doesn't get you points, it just saves
                        // you.
         ...
         * It does so by checking the center of the fireball as well as 4 points at the
         * corners
         * (NW, NE, SW, SE), of the fireball, to see if any of them overlapped with
         * non-transparent parts of the creature's image.
         * PRE: this should only be called if the creature has hatched and not been
         * killed., e is not null.
        ...
        public boolean fireballHasHitTheCreature(Egg e) {
            ...
            // System.out.println("fireball (" + fx + ", " + fy + "), creature(" + e.getX()
            // + ", " + e.getY() + ")");
            ...
            // to see if any of those coincide with non-transparent parts of the creature
            // image:
     ...
     * For example, pressing the up-arrow key causes an UpAction object to be
     * created.
     * Every time one of these action objects is created, it automatically calls its
     * actionPerformed method
     * to do the appropriate thing.
     * (For example, when the UpAction's actionPerformed method is called,
     * it calls the background's setUP method so that the background knows that the
     * person is trying to move up.)
     */
    ...
    public class UpAction extends AbstractAction {
        ...
        @Override
        public void actionPerformed(ActionEvent e) {
            background.setUp(pressed); // "setUP" means set the background to the "UP" condition, since it person just
