 ...
 * MessageBox
 * --------
 * Displays instructions in a box (used fire ball class as a template)
 */
public class MessageBox extends DragonGameShape {
    // we need to have access to the dragon and background to properly draw the
    // fireball:
    ...
    public MessageBox(Dragon d, Background b, int frame_width, int frame_height, String t) {
        this.dragon = d;
        this.background = b;
     ...
     * draw
     * ----
     * this draws the message box.
     * It also draws the "ready to fire meter" if the fireball isn't ready to fire
     * yet.
     * PRE: fired==true (otherwise it won't draw anything),
     * g != null
     * POST: none; nothing changes in the class, it just draws something on the
     * screen.
     */

    public void draw(Graphics g) {
        if (!this.readyToFire)
            drawReadyToFireMeter(g);
        ...
        int maxWidth = (FRAME_WIDTH / 6);
        ArrayList<String> lines = wrap(text, fm, maxWidth);
        int lineSpace = 20;
        int spacing = 0;
        ...
        System.out.println("msg draw");

        if (dragon.isFacingRight()) {
            g.setColor(Color.YELLOW);
            g.fillRect(((int) x + 40) - (offset / 2), ((int) y - (FRAME_HEIGHT * 80 / 100) / 2) - (offset / 2),
                    (FRAME_WIDTH / 3) + offset,
                    (FRAME_HEIGHT * 80 / 100) + offset);
            g.setColor(Color.BLACK);
            g.fillRect((int) x + 40, (int) y - (FRAME_HEIGHT * 80 / 100) / 2, FRAME_WIDTH / 3, FRAME_HEIGHT * 80 / 100);
            g.setColor(Color.WHITE);
            g.setFont(g.getFont().deriveFont(20f));

            for (int i = 0; i < lines.size(); i++) {
                g.drawString(lines.get(lines.size() - 1 - i), ((int) x + 40) + (offset / 2),
                        ((int) y - (FRAME_HEIGHT * 80 / 100) / 2) + (offset * 2) + spacing);
                spacing += lineSpace;
            }
            ...
            g.setColor(Color.YELLOW);
            g.fillRect(((int) x - (FRAME_WIDTH / 3) - 40) - (offset / 2),
                    ((int) y - (FRAME_HEIGHT * 80 / 100) / 2) - (offset / 2),
                    (FRAME_WIDTH / 3) + offset,
                    (FRAME_HEIGHT * 80 / 100) + offset);
            g.setColor(Color.BLACK);
            g.fillRect((int) x - (FRAME_WIDTH / 3) - 40, (int) y - (FRAME_HEIGHT * 80 / 100) / 2, FRAME_WIDTH / 3,
                    FRAME_HEIGHT * 80 / 100);
            g.setColor(Color.WHITE);
            g.setFont(g.getFont().deriveFont(20f));

            for (int i = 0; i < lines.size(); i++) {
                g.drawString(lines.get(lines.size() - 1 - i), ((int) x - (FRAME_WIDTH / 3) - 40) + (offset / 2),
                        ((int) y - (FRAME_HEIGHT * 80 / 100) / 2) + (offset * 2) + spacing);
                spacing += lineSpace;
            }
        }
    ...
    /**
     * ArrayList to hold all the words in a wrap
     * 
     * @param txt
     * @param fm
     * @param maxWidth
     * @return
     */
    public ArrayList<String> wrap(String txt, FontMetrics fm, int maxWidth) {
    ...
    public void drawReadyToFireMeter(Graphics g) {
         ...
         * g.drawString("seconds until you can fire again: " + secondsUntilReadyToFire,
         * x,y+200);
         */
     ...
     * nextLoc
     * -------
     * This determines the fireball's next location based on its current velocity
     * and acceleration, etc.
     * assuming it doesn't bounce off of anything
     * PRE: this.loc has been set
     * POST: none; this doesn't change anything, it only returns a
     * LocationAndVelocity object representing
     ...
     * move
     * ----
     * When this is called, it updates the message box's position based on dragon's
     * current
     * direction, etc.
     * PRE: loc has been set, background has been set.
     * POST: message box's location is updated.
     */
    @Override
    public void move() {
        if (!fired)
            return; // no point in moving if it hasn't been moved.
        ...
        // check for something to the left or right of you, and if you've got horizontal
        // velocity, about to hit it.
        ...
        // check if there's something right above or below you and you've got up or down
        // velocity, about to hit it.
        ...
        // if no bounces were made, but the next location would still put us inside the
        // landscape,
        ...
        this.loc = nextLoc();
        System.out.println("msg move");
        // if you stop moving, you've stopped firing, (so you won't be drawn, so you'll
        // disappear):
        if (Math.abs(this.loc.xv) < 0.05 && Math.abs(this.loc.yv) < 0.05)
            this.fired = false;
    }
     ...
     * fire
     * ----
     * this is called when you want to display the message box.
     ...
     * The inital location of the fireball or puff of smoke will be set to the
     * dragon's location
     * (or a little to the left or right depending on what direction the dragon is
     * facing.)
     * PRE: none
     * POST: if you were previously ready to fire, a timer gets started:
     * timerUntilReadyToFireAgain
     * fired == true
     * readyToFire == false
    ...
    public void fire(boolean b) {
        ...
        if (this.readyToFire == false) { // you tried to fire when you weren't ready, so we switch the fireball to smoke
            ...
            int timeUntilReadyToFire = 150; // it ticks down every tenth of a second
            timerUntilReadyToFireAgain = new Timer(timeUntilReadyToFire, new ActionListener() {
                @Override
                public void actionPerformed(ActionEvent e) {

                    // if (b == true) {
                    // secondsUntilReadyToFire = 0;
                    // }

                    secondsUntilReadyToFire -= 0.1;
                    if (secondsUntilReadyToFire <= 0.0)
                        getReadyToFire();
    ...
    public void getReadyToFire() {
        ...
        System.out.println("msg ready fire");
    }

    public boolean hasFired() {
        System.out.println("msg has fired");
        return this.fired;
    }

    public void setFired(boolean setToThis) {
        this.fired = setToThis;
        System.out.println("msg set fire");
    }
