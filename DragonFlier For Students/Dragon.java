...
public class Dragon extends DragonGameShape {
    ...
    // These booleans track whether the user is pushing the buttons to make the
    // dragon go in a certain direction:
    ...
    // This variable determines which dragon image in the above lists we're
    // currently using:
    private int whichDragon = 1;

    private int numberOfDraws = 0; // how many times the draw command has been called.
    // This is useful becayse every certain number of draws, we change the dragon
    // image.

    // Since the dragon needs to affect the background, we'll keep a reference to
    // it:
     ...
     * loadImages
     * ----------
     * This is just a helper method for the constructor, to load all the images we
     * need
    ...
    public void draw(Graphics g) {
        ...
        if (this.numberOfDraws > 15) {
            ...
            if (this.facingRight)
                this.dragonImage = nextRightDragonImage();
            else
                this.dragonImage = nextLeftDragonImage();
        }
        double x = loc.x;
        double y = loc.y;
     ...
     * nextLeftDragonImage
     * -------------------
     * This just gets the next image of the dragon facing left from the
     * LeftDragonImageList
    ...
    private BufferedImage nextLeftDragonImage() {
        ...
        if (this.whichDragon >= this.leftDragonImageList.size())
            this.whichDragon = 0;
     ...
     * -------------------
     * This just gets the next image of the dragon facing right from the
     * RightDragonImageList
     * PRE: images have been loaded into the list
     * POST: whichDragon is incremented.
     */
    private BufferedImage nextRightDragonImage() {
        this.whichDragon++; // whichCreature is essentially the index of which creature in the list to use.
        if (this.whichDragon >= this.rightDragonImageList.size())
            this.whichDragon = 0;
     ...
     * and if so, we'll bounce off of it (which means the background's velocity
     * should change)
    ...
    public void move() {
        ...
        // These leftOfX, rightOfX, aboveY, belowY are to get some points around the
        // dragon
        ...
        // in case vertical velocity is 0, but they're right near the top, pushing up,
        // don't let them:
